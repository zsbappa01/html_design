...
function laborator_display_header_top_bar($widget)
{
	...
	if($widget == 'laborator_cart_totals' && function_exists('WC'))
	{
		...
		<nav>
			<li>
				<a href="<?php echo wc_get_cart_url(); ?>"><?php echo sprintf(__('Cart totals: <span id="cart-totals">%s</span>', 'aurum'), WC()->cart->get_cart_total()); ?></a>
			</li>
		</nav>
		<?php
	}
...
function aurum_show_header_cart_icon( $size = array( 24, 24 ) ) {
	...
	<section class="cart-info">
		<a class="cart-counter<?php echo $cart_items_count ? ' has-notifications' : ''; echo $cart_items_count == 0 ? " cart-zero" : ''; ?>" href="<?php echo wc_get_cart_url(); ?>">
